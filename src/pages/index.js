  ...
  return (
    <section className={styles.labSection}>
      <div className="container">
        ...
        <LabArchitecture className={styles.labDiagram} role="img" aria-label="Architecture diagram of the full lab: an OPNsense firewall splitting a WAN segment holding the Kali attacker box from a LAN segment holding the domain controller, issuing CA, AD FS server, offline root CA, and an Ubuntu Docker host running Wazuh, Grafana and Gitea, with an OpenVAS vulnerability scanner and a Suricata sensor watching both segments, all on one laptop, and the domain controller syncing outward to a Microsoft Entra ID cloud directory." />
      </div>
    </section>
  );
}
