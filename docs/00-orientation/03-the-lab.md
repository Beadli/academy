...
<LabArchitecture role="img" aria-label="Architecture diagram of the full lab on one computer: OPNsense firewall splitting a WAN segment (Kali attacker box) from a LAN segment holding two domain controllers replicating with each other, an issuing CA, an AD FS server, an offline root CA, and an Ubuntu Docker host running Wazuh, Grafana, Gitea, Keycloak, a step-ca certificate authority and an nginx reverse proxy, with an OpenVAS vulnerability scanner and a Suricata sensor watching both segments, and the first domain controller syncing outward to a Microsoft Entra ID cloud directory." style={{width: '100%', height: 'auto'}} />
